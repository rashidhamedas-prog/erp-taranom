#!/usr/bin/env python3
"""Paramiko overlay: invoice UX + fabric/freight/dashboard v193."""
...
STAMP_FILE = ".sftp-deploy-stamp-erp2-v193"
...
FILES = [
    ...
    "server/scripts/test-invoice-ux-v193.js",
]
...
def main() -> None:
    ...
    pull_code, pull_text = run(c, f"cd {APP} && git pull --ff-only origin claude/claude-md-docs-2ssrpy")
    if pull_code != 0:
        print("git pull failed; overlay SFTP v193")
        sftp = c.open_sftp()
        for rel in FILES:
            local = ROOT / rel
