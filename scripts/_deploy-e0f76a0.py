#!/usr/bin/env python3
"""Deploy e0f76a0 R13 full-reverse to Iran."""
...
def main() -> None:
    ...
    run(c, f"cd {APP} && git stash push -u -m 'pre-deploy-e0f76a0' || true")
    ...
    run(c, f"cd {APP} && grep -n \"erp-taranom-v121\" server/public/sw.js | head -2")
    run(c, f"cd {APP} && grep -n \"voidCycleCount\\|void-opening\\|void-payment\\|VOIDABLE_VOUCHER\" server/routes/*.js server/lib/*.js server/lib/inventory/*.js | head -20")
    c.close()
    print("DONE")
