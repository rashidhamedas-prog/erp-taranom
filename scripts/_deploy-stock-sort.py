#!/usr/bin/env python3
"""Deploy stock-desc catalog sort to Iran."""
...
def main() -> None:
    ...
    run(
        c,
        f"cd {APP} && git fetch origin claude/claude-md-docs-2ssrpy 2>&1 | tail -5 && "
    ...
    code, out = run(
        ...
        "grep -n 'CAST(p.stock\\|CAST(stock' server/routes/products.js server/routes/b2b.js; "
        "grep -n 'بیشترین موجودی' server/public/index.html | head -3; "
        "git rev-parse --short HEAD",
    )
    if "CAST(p.stock" not in out or "v102" not in out:
        print("VERIFY FAILED")
        sys.exit(1)
