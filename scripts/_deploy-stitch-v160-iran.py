#!/usr/bin/env python3
"""SFTP overlay stitch v160. VPS cannot fetch GitHub. Do not replace db.js (VPS-dirty)."""
...
STAMP_HASH = "7dd5481"
STAMP_FILE = ".sftp-deploy-stamp-stitch-v160"
...
FILES = [
    ...
    "server/scripts/test-acc-stitch-p2.js",
    "server/scripts/test-portal.js",
...
def main() -> None:
    ...
    patch_local = ROOT / "scripts/_patch-product-cat-coa.js"
    sftp.put(str(patch_local), f"{APP}/scripts/_patch-product-cat-coa.js")
    print("PUT scripts/_patch-product-cat-coa.js")
    sftp.close()

    code, patch_out = run(c, f"node {APP}/scripts/_patch-product-cat-coa.js", timeout=30)
    if code != 0 or ("DB_COA_PATCHED" not in patch_out and "DB_COA_ALREADY" not in patch_out):
        c.close()
        raise SystemExit("db.js patch failed")
    ...
    run(
        c,
        f"cd {APP} && echo {STAMP_FILE}={stamp} hash={STAMP_HASH} > {STAMP_FILE} && cat {STAMP_FILE}",
        timeout=30,
    )
    c.close()
    print("DEPLOY_DONE", STAMP_HASH)
