#!/usr/bin/env python3
"""Paramiko overlay: invoice fabric/journal/print v194."""
...
STAMP_FILE = ".sftp-deploy-stamp-invoice-v194"
...
FILES = [
    ...
    "server/lib/inventory/fabric-rolls.js",
    "server/routes/invoices.js",
    "server/routes/purchases.js",
    "server/scripts/test-invoice-docs-v194.js",
]
...
def main() -> None:
    ...
    pull_code, _pull_text = run(c, f"cd {APP} && git pull --ff-only origin claude/claude-md-docs-2ssrpy")
    if pull_code != 0:
        print("git pull failed; overlay SFTP v194")
        sftp = c.open_sftp()
        for rel in FILES:
            local = ROOT / rel
        ...
        sftp.close()
    run(c, "cd /home/taranom/crm-taranom/server && pm2 restart erp-taranom --update-env --max-memory-restart 1024M 2>&1 | tail -12")
    _code, _ = run(
        c,
