#!/usr/bin/env python3
"""SFTP overlay stitch v165 (POS). VPS cannot fetch GitHub. Do not replace db.js (VPS-dirty)."""
...
STAMP_FILE = ".sftp-deploy-stamp-stitch-v165"
...
FILES = [
    ...
    "server/scripts/test-pos-stitch-p8.js",
    "server/server.js",
    "server/sync/capture.js",
    "server/sync/tables.js",
    "scripts/_patch-pos-schema.js",
]
...
def main() -> None:
    ...
    code, patch_out = run(c, f"node {APP}/scripts/_patch-pos-schema.js", timeout=30)
    if code != 0 or not any(x in patch_out for x in ("DB_POS_PATCHED", "DB_POS_ALREADY")):
        c.close()
        raise SystemExit("db.js POS schema patch failed")
