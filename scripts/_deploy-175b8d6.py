#!/usr/bin/env python3
"""Deploy feef58d / 175b8d6 to Iran central."""
...
def main() -> None:
    ...
    run(c, f"cd {APP} && git rev-parse --short HEAD && git log -2 --oneline")
    ...
    run(c, f"cd {APP} && grep -n \"erp-taranom-v119\" server/public/sw.js | head -1")
    run(c, f"cd {APP} && grep -n \"hasSupplierLedger\\|receiptOpChooser\\|sms_rules\" server/routes/accounting.js server/public/index.html server/sync/tables.js | head -20")
    c.close()
    print("DONE")
