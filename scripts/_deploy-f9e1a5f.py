...
run(
    ...
    "git stash push -m deploy-f9e1a5f -- server/package-lock.json server/routes/products.js 2>/dev/null; "
    ...
    "curl -sSI http://127.0.0.1:3000/acc-nav.js?v=77 | tr -d '\\r' | grep -iE 'HTTP/|cache-control|content-type' && "
    "curl -sS http://127.0.0.1:3000/acc-nav.js?v=77 | grep -E \"title: '\" | head -8 && "
    "grep -n erp-taranom-v114 server/public/sw.js"
)
c.close()
print("DONE")
