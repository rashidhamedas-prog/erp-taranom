#!/usr/bin/env python3
"""Retry Iran deploy: stash local drift, ff-only pull, restart."""
...
def main() -> None:
    ...
    run(c, f"cd {APP} && git status -sb && git stash push -u -m 'pre-deploy-feef58d' -- server/routes/products.js server/package-lock.json || true")
    ...
    run(c, f"cd {APP} && grep -n \"erp-taranom-v119\" server/public/sw.js | head -2")
    run(c, f"cd {APP} && grep -n \"hasSupplierLedger\" server/routes/accounting.js | head -2")
    run(c, f"cd {APP} && grep -n \"receiptOpChooser\" server/public/index.html | head -2")
    run(c, f"cd {APP} && grep -n \"sms_rules\" server/sync/tables.js | head -2")
    c.close()
    print("DONE")
