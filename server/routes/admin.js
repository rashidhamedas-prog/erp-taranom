...
router.post('/restore-product-stock-wipe', auth, adminOnly, centralOnly, (req, res) => {
  ...
  // Always allow manual re-run (fill-empty only; safe to repeat)
  ...
  try { audit(req.user.id, 'restore', 'products', null, `بازیابی موجودی/قیمت/کد پس از وایپ عکس: ${JSON.stringify(summary)}`); } catch (_) {}
