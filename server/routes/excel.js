...
const DEFINITIONS = {
  ...
  'opening-recv-cheques': {
    ...
    guide: ['طرف حساب باید با شناسه، کد شخص، تلفن یا نام دقیق یک ردیف موجود در اشخاص مطابقت کند — نام آزاد ثبت نمی‌شود.'],
  },
  'opening-pay-cheques': {
  ...
  if (entity.startsWith('opening-')) {
    ...
    rows.forEach((r, i) => {
      ...
      out.push(action('/cheque-records', {
        ...
        party_id: requireRef(rowNo, 'طرف حساب', findOne(db, 'parties', field(r, 'طرف حساب*', 'طرف حساب', 'party_id', 'party_name', 'کد شخص'), ['id', 'person_code', 'full_name', 'company_name', 'biz', 'phone'])).id,
