...
router.post('/', auth, adminOrAccounting, (req, res) => {
  const { name, code, entity_type, description } = req.body;
  ...
  const result = db.prepare('INSERT INTO party_groups (code,name,entity_type,description) VALUES (?,?,?,?)')
    .run(c, String(name).trim(), entity_type || 'all', description || '');
...
router.put('/:id', auth, adminOrAccounting, (req, res) => {
  ...
  const { name, entity_type, description, active } = req.body;
  db.prepare('UPDATE party_groups SET name=?,entity_type=?,description=?,active=? WHERE id=?')
    .run(name || row.name, entity_type || row.entity_type, description ?? row.description, active != null ? (active ? 1 : 0) : row.active, req.params.id);
