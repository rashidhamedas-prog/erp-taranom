...
| آخرین commit | (پس از commit قوانین یکپارچه) |
| نسخه وب/دسکتاپ | **`1.0.11`** / SW `v42` |
| اندروید | **`2.0.9`** (versionCode 11) — **توزیع محلی فقط** |
| وضعیت سرور | ⏳ نمایندگان + ریال یکپارچه — در حال deploy |
...
- **Commit:** (همین جلسه)
- **خلاصه:**
  - رفع خطای نمایندگان فروش: `/reps/alerts` به‌جای `invoices.cheque_status` از `settlements.cheque_status` استفاده می‌کند؛ UI با Promise.allSettled مقاوم شد.
  - واحد پول کل برنامه فقط **ریال** (نمایش و ورودی؛ حذف حالت تومان).
  - جداکننده سه‌رقمی خودکار برای همه فیلدهای مبلغی (کلاس money + تشخیص شناسه).
  - آیکون‌های منو به Lucide خطی یکدست؛ emoji خام دیگر در سایدبار نمایش داده نمی‌شود.
  - دستیار AI برای همه نقش‌ها (داده‌های شخصی)؛ مشاور شرکتی فقط مدیر.
- **فایل‌های کلیدی:** `server/routes/rep-management.js`, `server/public/index.html`, `server/public/sw.js`
- **Deploy:** ⏳
