...
- **شاخه:** `claude/claude-md-docs-2ssrpy`
- **Commit:** `18e2d38`
- **خلاصه:** لوکال FF به tip؛ روی VPS `sharp@0.35.0` به‌خاطر CPU لود نمی‌شد → recover به `0.33.5` (`REQUIRE_OK`). سپس SFTP هدفمند P3+P4 بدون `npm install` و بدون `--update-env`.
- **فایل‌های کلیدی:** `scripts/_deploy-sync-tip-sftp.py`, `server/lib/production/{variance,engine,labor,overhead}.js`
- **Deploy:** ✅ tip overlay `2d624c9`؛ stamp `.sftp-deploy-stamp-sync-tip`; root/health/ready=200؛ SW `v144`؛ sharp=`0.33.5`
- **یادداشت:** Git HEAD روی VPS هنوز `c2add63` (dirty overlay)؛ blind pull انجام نشد. waiver دائمی sharp برقرار است.
